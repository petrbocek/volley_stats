...
/* ─── SOUPEŘOVA STRANA ───
   Co se stalo na druhé straně sítě: jeho chyba (náš bod) a jeho vlastní bod
   (jejich bod). Nemá to naši hráčku, takže vlastní tabulka a vlastní RPC.
   Zapisuje se přírůstkově jako všechno ostatní, kvůli dvěma zapisovatelům
   u jednoho zápasu (#74).

   Bez „bodu soupeře" by odvozené skóre soupeře podhodnocovalo: jeho zakončení
   v otevřené výměně se u nás neprojeví nijak — obrana ani přihrávka mezi
   akcemi nejsou, takže tu výměnu nemá co zapsat. */
const SOUPER_POLE=[
  {pole:'pocet',label:'Chyba soupeře',znak:'+',cls:'plus',komu:'náš bod'},
  {pole:'body', label:'Bod soupeře',  znak:'−',cls:'minus',komu:'jejich bod'},
];
const souperDirty={};         // `${zapasId}_${set}_${pole}` -> zobrazená hodnota
const souperPending={};       // totéž -> neodeslaná změna
const souperDebounce={};

function souperKey(zapasId,set,pole){return `${zapasId}_${set}_${pole}`;}

function souperUlozene(zapasId,set,pole){
...
function souperHodnota(zapasId,set,pole){
  const k=souperKey(zapasId,set,pole);
  return k in souperDirty?souperDirty[k]:souperUlozene(zapasId,set,pole);
}

function souperZapas(zapasId,pole){
  let n=0;
  for(let set=1;set<=SETU;set++)n+=souperHodnota(zapasId,set,pole);
  return n;
}

function bumpSouper(zapasId,pole,delta=1,opts={}){
  if(!isLoggedIn()){toast('Na zapisování se přihlas (🔒 nahoře)','error');return false;}
  const set=opts.set||state.liveSet;
  const k=souperKey(zapasId,set,pole);
  const puvodni=souperHodnota(zapasId,set,pole);
  const nova=Math.max(0,puvodni+delta);
  if(nova===puvodni)return false;
  souperDirty[k]=nova;
  souperPending[k]=(souperPending[k]||0)+(nova-puvodni);
  if(souperPending[k]===0)delete souperPending[k];
  ...
  if(!opts.bezUndo){
    if(delta>0){
      ...
      for(let i=undoStack.length-1;i>=0;i--){
        const u=undoStack[i];
        if(u.typ==='souper'&&u.zapasId===zapasId&&u.set===set&&u.pole===pole){
...
async function flushSouper(key,opts={}){
  const delta=souperPending[key];
  if(!delta)return;
  clearTimeout(souperDebounce[key]);
  delete souperDebounce[key];
  const [zapasId,set,pole]=[...key.split('_').slice(0,2).map(Number),key.split('_')[2]];
  delete souperPending[key];
  try{
    const r=await apiRpc('vb_zapis_souper',
      {p_zapas:zapasId,p_set:set,p_pole:pole,p_delta:delta},opts);
    if(r&&typeof r==='object'){
      // server vrací oba sloupce včetně toho, co zapsal někdo druhý
      const idx=state.chybySouperu.findIndex(c=>c.zapas_id===zapasId&&(c.set_cislo||1)===set);
      const radek={zapas_id:zapasId,set_cislo:set,pocet:r.pocet||0,body:r.body||0};
      if(idx>=0)state.chybySouperu[idx]=radek;else state.chybySouperu.push(radek);
      SOUPER_POLE.forEach(({pole:p})=>{
        const kk=souperKey(zapasId,set,p);
        if(!(kk in souperPending))souperDirty[kk]=radek[p];
      });
      prekresliSouper();
    }
  }catch(e){
    souperPending[key]=(souperPending[key]||0)+delta;   // vrátit do fronty, ať se to neztratí
    toast('Chyba uložení: '+e.message,'error');
  }
}

function flushVsechnySouper(opts={}){
...
function prekresliSouper(){
  ...
  SOUPER_POLE.forEach(({pole})=>{
    ...
    if(el)el.textContent=souhrnCelyZapas
      ?souperZapas(zapasId,pole)
      :souperHodnota(zapasId,state.liveSet,pole);
  });
}

/* ─── POSTAVENÍ NA HŘIŠTI ───
   Zóny jako ve volejbale: 4-3-2 u sítě, 5-6-1 vzadu, zóna 1 podává. Libero
   se do zóny neukládá — pozná se podle pozice a stojí stranou, protože se
...
function postaveniSetu(zapasId,set){
  const m=new Map();
  state.postaveni.filter(p=>p.zapas_id===zapasId&&(p.set_cislo||1)===set)
    .forEach(p=>m.set(p.zona,p.hrac_id));
  return m;
}

// Libero je role v konkrétním zápase, ne vlastnost hráčky: pozice v kartě
// hráčky o tom nerozhoduje, nominuje se do sestavy zápasu.
function jeLibero(zapasId,hracId){
  return !!state.zapasHraci.find(zh=>zh.zapas_id===zapasId&&zh.hrac_id===hracId)?.libero;
}

function liberaSestavy(zapasId){
  const sid=currentSeasonId()||state.zapasy.find(z=>z.id===zapasId)?.sezona_id||0;
  return serazenaSestava(zapasId,hraciVSezoně(sid)).filter(h=>jeLibero(zapasId,h.id));
}

async function prepniLibero(zapasId,hracId){
  if(!isLoggedIn()){toast('Na změny se přihlas (🔒 nahoře)','error');return;}
  const zh=state.zapasHraci.find(x=>x.zapas_id===zapasId&&x.hrac_id===hracId);
  if(!zh)return;
  const nove=!zh.libero;
  try{
    await apiUpsert('vb_zapas_hraci',{zapas_id:zapasId,hrac_id:hracId,libero:nove},'zapas_id,hrac_id');
    zh.libero=nove;
    // nominované libero na hřišti nestojí, tak ji z postavení vyndáme
    if(nove)for(let set=1;set<=SETU;set++){
      const m=postaveniSetu(zapasId,set);
      const zona=[...m.entries()].find(([,id])=>id===hracId)?.[0];
      if(zona)vyndejZeZony(zapasId,set,zona);
    }
    prekresliLive(zapasId);
    toast(nove?'Nominováno jako libero':'Libero zrušeno','success');
  }catch(e){toast('Chyba: '+e.message,'error');}
}

function v2PrepniLibero(){
...
function lavicka(zapasId,set){
  const sid=currentSeasonId()||state.zapasy.find(z=>z.id===zapasId)?.sezona_id||0;
  const naHristi=[...postaveniSetu(zapasId,set).values()];
  return serazenaSestava(zapasId,hraciVSezoně(sid))
    .filter(h=>!naHristi.includes(h.id)&&!jeLibero(zapasId,h.id));
}

// Všechno postavení jde jednou cestou: stav se upraví v paměti a odešle celý.
// Dva požadavky za sebou (smaž tam, zapiš sem) můžou skončit v půlce a nechat
// set bez hráčky.
function postavDoZony(zapasId,set,zona,hracId){
  if(!isLoggedIn()){toast('Na změny se přihlas (🔒 nahoře)','error');return;}
  const m=postaveniSetu(zapasId,set);
  [...m.entries()].forEach(([z,id])=>{if(id===hracId)m.delete(z);});   // jen jednou na hřišti
  m.set(zona,hracId);
  state.postaveni=state.postaveni.filter(p=>!(p.zapas_id===zapasId&&(p.set_cislo||1)===set));
  state.postaveni.push(...[...m.entries()].map(([z,id])=>({zapas_id:zapasId,set_cislo:set,zona:z,hrac_id:id})));
  ulozPostaveni(zapasId,set);
  prekresliLive(zapasId);
}

function vyndejZeZony(zapasId,set,zona){
  if(!isLoggedIn()){toast('Na změny se přihlas (🔒 nahoře)','error');return;}
  state.postaveni=state.postaveni.filter(p=>!(p.zapas_id===zapasId&&(p.set_cislo||1)===set&&p.zona===zona));
  ulozPostaveni(zapasId,set);
  prekresliLive(zapasId);
}

// Základní postavení se dá převzít z předchozího setu — naklikávat šestku
// znovu mezi sety je zbytečná práce, měnívá se jeden dva posty.
async function prevezmiPostaveni(zapasId,set){
  const zdroj=set-1;
  if(zdroj<1)return;
  const m=postaveniSetu(zapasId,zdroj);
  if(!m.size){toast('Předchozí set žádné postavení nemá','error');return;}
  state.postaveni=state.postaveni.filter(p=>!(p.zapas_id===zapasId&&(p.set_cislo||1)===set));
  state.postaveni.push(...[...m.entries()].map(([zona,hrac_id])=>({zapas_id:zapasId,set_cislo:set,zona,hrac_id})));
  await ulozPostaveni(zapasId,set);
  prekresliLive(zapasId);
...
function skoreSetu(zapasId,set){
  ...
  const soucet=pole=>hraci.reduce((n,h)=>
    n+pole.reduce((m,f)=>m+getStatVal(zapasId,h.id,f,set),0),0);
  return {
    nase:soucet(SKORE_NASE)+souperHodnota(zapasId,set,'pocet'),
    jejich:soucet(SKORE_JEJICH)+souperHodnota(zapasId,set,'body'),
  };
}

// Kolik toho v setu vůbec je. Prázdný set nemá smysl ukazovat jako 0:0 —
// vypadalo by to, že se hraje a nikdo neskóruje.
function setMaData(zapasId,set){
  const s=skoreSetu(zapasId,set);
  return s.nase>0||s.jejich>0;
}

// Kontrola proti tomu, co je zapsané ve „Výsledku". Když se odvozené skóre
// rozchází, něco v zápisu chybí — a musí to být vidět hned, ne až po sezóně
// (#84). Vrací null, když není s čím porovnávat.
function kontrolaSkore(zapasId,set){
  const z=state.zapasy.find(z=>z.id===zapasId);
  if(!z)return null;
  const zapsaneMy=z[`set${set}_my`],zapsaneOni=z[`set${set}_oni`];
  if(zapsaneMy==null||zapsaneOni==null)return null;
  const odvozene=skoreSetu(zapasId,set);
  const rozdilMy=odvozene.nase-zapsaneMy,rozdilOni=odvozene.jejich-zapsaneOni;
  return {odvozene,zapsane:{nase:zapsaneMy,jejich:zapsaneOni},
          rozdilMy,rozdilOni,sedi:rozdilMy===0&&rozdilOni===0};
}

// Stav utkání = kolik setů je rozhodnutých. Zapsaný výsledek má přednost před
// odvozeným; bez něj se set počítá za rozhodnutý podle pravidla (25, v pátém
// 15, vždy o dva) — dřív ne, aby se rozehraný set nehlásil jako vyhraný.
function setRozhodnuty(zapasId,set){
  const z=state.zapasy.find(z=>z.id===zapasId);
...
function skoreHtml(zapasId){
  ...
  const varovani=k&&!k.sedi
    ? `<div class="skore-nesedi" title="Odvozeno z akcí vs. zapsáno ve Výsledku">
         ⚠ Zapsáno ${k.zapsane.nase}:${k.zapsane.jejich} — z akcí vychází ${k.odvozene.nase}:${k.odvozene.jejich}
       </div>`
    : '';
  const u=stavUtkani(zapasId);
  return `<div class="skore">
    <div class="skore-utkani">Sety <strong>${u.my}:${u.oni}</strong></div>
    <div class="skore-hlavni">
      <span class="skore-popis">${set}. set</span>
      <span class="skore-cisla"><span id="skore-nase">${s.nase}</span><span class="skore-dvojtecka">:</span><span id="skore-jejich">${s.jejich}</span></span>
    </div>
...
// znamená, že hráčka na okamžik existuje ve dvou zónách naráz. Celé postavení
// se proto vyměňuje jednou RPC v jedné transakci.
async function ulozPostaveni(zapasId,set){
  if(!isLoggedIn())return;
  const radky=[...postaveniSetu(zapasId,set).entries()].map(([zona,hrac_id])=>({zona,hrac_id}));
  try{
    await apiRpc('vb_uloz_postaveni',{p_zapas:zapasId,p_set:set,p_postaveni:radky});
  }catch(e){toast('Postavení se neuložilo: '+e.message,'error');}
}

function obnovPostaveni(zapasId,set,dvojice){
  state.postaveni=state.postaveni.filter(p=>!(p.zapas_id===zapasId&&(p.set_cislo||1)===set));
  state.postaveni.push(...dvojice.map(([zona,hrac_id])=>({zapas_id:zapasId,set_cislo:set,zona,hrac_id})));
  ulozPostaveni(zapasId,set);
}
...
function v2OtevriZonu(zapasId,zona){
  const set=state.liveSet;
...
function v2OtevriObsazeni(zapasId,zona,nahrazuje=0){
  ...
  document.getElementById('v2-zona-title').textContent=
    nahrazuje?`Střídání do zóny ${zona}`:`Kdo stojí v zóně ${zona}?`;
  const volne=lavicka(zapasId,state.liveSet);
  const el=document.getElementById('v2-zona-obsah');
  el.innerHTML=volne.length
    ? volne.map(h=>playerCard(h,{
    ...
    : '<div class="empty" style="padding:20px"><span class="empty-icon">👥</span><div class="empty-text">Na lavičce nikdo nezbyl</div><div>Přidej hráčku do sestavy v seznamu</div></div>';
...
function v2VyberDoZony(hracId){
  ...
  if(zona)postavDoZony(zapasId,state.liveSet,zona,hracId);
...
function renderLive2(zapasId){
  ...
  // Dvě tlačítka, každé pojmenované. Samotné +/− by u soupeřovy strany nešlo
  // přečíst: „+" je náš bod z jeho chyby, „−" jeho vlastní bod — dvě různé
  // události, ne dvě kvality téže akce.
  const chyby=`<div class="v2-souper">${SOUPER_POLE.map(sp=>`
    <button class="v2-souper-btn ${sp.cls}" onclick="bumpSouper(${zapasId},'${sp.pole}')"
        title="${sp.label} — ${sp.komu}; zpět přes lištu dole">
      <span class="v2-souper-nazev">${sp.znak} ${sp.label}</span>
      <span class="v2-souper-cislo" id="v2-souper-${sp.pole}">${souhrnCelyZapas?souperZapas(zapasId,sp.pole):souperHodnota(zapasId,set,sp.pole)}</span>
    </button>`).join('')}</div>`;

  const prepinacZobrazeni=`<div class="v2-zobrazeni">
    <button class="v2-zobrazeni-btn${v2Hriste?'':' aktivni'}" onclick="if(v2Hriste)v2PrepniHriste()">Seznam</button>
    <button class="v2-zobrazeni-btn${v2Hriste?' aktivni':''}" onclick="if(!v2Hriste)v2PrepniHriste()">Hřiště</button>
  </div>`;
...
function prekresliV2Cisla(){
  ...
  const hraci=serazenaSestava(zapasId,hraciVSezoně(sezona_id));
  if(v2Hriste){
    // na hřišti jsou čísla uvnitř karet zón, překreslí se s nimi
    renderLive2(zapasId);
    return;
  }
  hraci.forEach(h=>{
...
function undoPopisek(){
  ...
  if(u.typ==='souper'){
    ...
    const sp=SOUPER_POLE.find(x=>x.pole===(u.pole||'pocet'));
    return `${sp?sp.label:'Soupeř'}${jinySet}`;
  }
...
function flushVse(opts={}){
  return Promise.all([flushAllStats(opts),flushVsechnySouper(opts)]);
}
...
async function refreshLiveStats(){
  ...
  try{
    ...
    (chyby||[]).forEach(row=>{
      ...
      SOUPER_POLE.forEach(({pole})=>{
        const k=souperKey(row.zapas_id,set,pole);
        if(k in souperPending)return;           // tohle si drží uživatel
        souperDirty[k]=row[pole]||0;
      });
    });
    prekresliV2Cisla();
    prekresliSouper();
...
function renderStatistiky(){
  ...
  const souper=pole=>d.zapasIds.reduce((n,zid)=>n+(selSet
    ?souperHodnota(zid,selSet,pole)
    :souperZapas(zid,pole)),0);
  html+=`<div class="stats-chyby">Soupeř${selSet?` (${selSet}. set)`:''}:
    ${SOUPER_POLE.map(sp=>`${sp.label} <strong>${souper(sp.pole)}</strong>`).join(' · ')}</div>`;
  el.innerHTML=html;
}
...
function detailTymu(tymId){
  ...
  return {t,zapasy,done,vyhry,prohry,setyMy,setyOni,
          ...
          chyby:zapasy.reduce((n,z)=>n+souperZapas(z.id,'pocet'),0),
          bodySoupere:zapasy.reduce((n,z)=>n+souperZapas(z.id,'body'),0)};
}

function otevriTymDetail(tymId){
  const p=detailTymu(tymId);
  if(!p){toast('Detail týmu se nepodařilo sestavit','error');return;}
  const {t,zapasy,done,vyhry,prohry,setyMy,setyOni,rows,tot,chyby,bodySoupere}=p;
...
async function deleteZapas(id){
  ...
  const chyby=souperZapas(id,'pocet')+souperZapas(id,'body');
  const chybyText=chyby?` Zmizí i ${chyby} zapsaných akcí soupeře.`:'';
  ...
  try{
    ...
    for(let set=1;set<=SETU;set++){
      SOUPER_POLE.forEach(({pole})=>{
        ...
        delete souperPending[k];clearTimeout(souperDebounce[k]);delete souperDirty[k];
      });
    }
...
function editVysledek(zapasId,finish=false){
  ...
  for(let i=1;i<=5;i++){
    ...
    const maData=odv.nase>0||odv.jejich>0;
    const napoveda=maData
      ?`<div class="vysledek-odvozene" id="odv-set${i}">Z akcí vychází <strong>${odv.nase}:${odv.jejich}</strong></div>`
      :'';
